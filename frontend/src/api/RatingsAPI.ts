...
const API_URL = import.meta.env.VITE_API_URL || 'https://cinenicheback-arfmceaefxg5dqb9.westcentralus-01.azurewebsites.net/api';
...
export async function submitRating(
  ...
  const res = await fetch('https://cinenicheback-arfmceaefxg5dqb9.westcentralus-01.azurewebsites.net/api/Ratings/Add', {
...
export async function getAverageRating(showId: string): Promise<number | null> {
  try {
    const response = await fetch(
      `https://cinenicheback-arfmceaefxg5dqb9.westcentralus-01.azurewebsites.net/api/Ratings/Average/${showId}`
    );
    if (!response.ok) {
